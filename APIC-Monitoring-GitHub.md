...
```mermaid
flowchart TD
    APIC[API Connect Cluster]

    APIC --> MGMT[Management]
    APIC --> PORTAL[Portal]
    APIC --> ANALYTICS[Analytics]
    APIC --> GATEWAY[Gateway]

    MGMT --> DB[Postgres / Db]
    MGMT --> APIM[Apim]
    MGMT --> TASK[Task Manager]

    PORTAL --> PWWW[Portal WWW]
    PORTAL --> PDB[Portal DB]
    PORTAL --> NGINX[Portal Nginx]

    ANALYTICS --> STORAGE[Storage]
    ANALYTICS --> DIRECTOR[Director]
    ANALYTICS --> INGESTION[Ingestion]
    ANALYTICS --> MTLS[Mtls-gw]
    ANALYTICS --> MASTER[Storage-os-master]

    GATEWAY --> DP[DataPower / gwv6]
```

> [!NOTE]
> The diagram organizes the components referenced in this document by subsystem. It is provided as a visual navigation aid.
